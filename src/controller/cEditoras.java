...
public class cEditoras {
    ...
    public List<mEditoras> pesquisar(String texto, int filtro) {
        ...
        try {
            ...
            if (filtro == 0) {
                stmt = conn.prepareStatement("SELECT * FROM editoras WHERE "
                        + "id_editora = ?");
                stmt.setString(1, texto);
                rs = stmt.executeQuery();
            } else {
                stmt = conn.prepareStatement("SELECT * FROM editoras WHERE nome "
                        + "like ?");
                stmt.setString(1, "%" + texto + "%");
                rs = stmt.executeQuery();
    ...
    public void Excluir (mEditoras modelE) {
        ...
        try {
            stmt = conn.prepareStatement("DELETE FROM editoras where "
                    + "id_editora = ?");
            stmt.setInt(1, modelE.getId_editora());

            stmt.executeUpdate();

            JOptionPane.showMessageDialog(null, "Editora excluida com "
                    + "sucesso!"
            );
